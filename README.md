...
- App Store privacy manifest declaring no tracking, no collected data types, and app-only UserDefaults access
...
- Maintenance Center with daily learning health metrics and direct links to missing-audio repair, feedback, rare glyphs, study, and review history
...
Cards whose audio file names exist in the database but whose local files are missing are treated as audio issues. Study shows them as missing audio, the editor clears broken references on save, and the missing-audio queue can clean and refill them.
...
- [x] Privacy manifest for App Store required-reason API review
